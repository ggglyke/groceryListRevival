...
export default class List extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isLoading: true,
      dbProducts: [],
      dbProductsloaded: false,
      magasins: [],
      magasinsLoaded: false,
      editingTitle: false,
      listNewTitle: "",
      listDeleted: false,
      showModal: false,
  ...
  quickAdd(e) {
    const defaultAisle = this.state.aisles.find((rayon) => rayon.isDefault);
    const defaultAisleId = defaultAisle._id;
    this.setState(
      {
        isLoading: true,
        newProductAisle: defaultAisleId,
        alsoAddToDatabase: false,
        productName: "",
      },
                    ...
                    <ul className="list-group">
                      {rayonList.map((rayon, index) => (
                        <React.Fragment key={rayon.title}>
                          <li
                            className="list-group-item py-2 fw-600 bg-light rayonTitle"
                            key={rayon.title}
                          >
                            {rayon.title}
                          </li>
                          {rayon.products.map((product, index) => (
                            ...
                            >
                              ...
                              this.state.customizing._id === product._id ? (
                                <Form className="my-2 d-flex justify-content-between flex-fill">
                                  <Form.Group
                                    className="flex-fill"
          ...
          <Form onSubmit={this.submitNewProduct}>
            <Modal.Body>
              ...
              <Form.Group controlId="newProductAisle" className="mb-3">
                ...
                {aisles.length < 1 ? (
                  <FormText className="text-small text-danger ms-2">
